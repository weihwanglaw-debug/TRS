...
- TRS payment quotes and persisted registrations/payments use SGD.
- Currency submitted by the browser is not authoritative and is ignored by server-side pricing.
...
- Stripe refund statuses remain `P` until Stripe reports `succeeded`; `failed` and `canceled` map to `F`. Pending refunds do not change payment-item or participant/group cancellation state.
- `refund.created`, `refund.updated`, `refund.failed`, and legacy `charge.refund.updated` webhook processing reconcile pending refund state idempotently. A pending cancellation-with-refund releases the participant/group slot only after Stripe reports success.
- The deployed Stripe webhook endpoint must subscribe to `refund.created`, `refund.updated`, `refund.failed`, and `charge.refunded`; legacy `charge.refund.updated` remains supported for older configurations.
- Multi-item cancellation state is derived from all related refund rows so webhook arrival order cannot hide a failed refund. Any failed cancellation refund keeps the registration in `RF`; any pending whole-registration refund keeps it in `CP`.
...
- A result from an earlier knockout round cannot be amended while a later/current round exists. The later round must be reset first. Group-stage results cannot be amended after the knockout stage is generated until the knockout stage is reset.
...
- Participant details and their derived group/payment display names can be amended only before a fixture exists for that program. Reset the fixture before changing participant details.
